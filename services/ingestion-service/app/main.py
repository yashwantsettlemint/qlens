...
async def _validate(rows: list[dict], default_source: str) -> list[RowResult]:
    vendors, pos = await fetch_lookups()
...
async def upload_csv(file: UploadFile = File(...)) -> ValidateResponse:
    ...
    results = await _validate(rows, default_source="csv")
...
async def commit_csv(req: CommitRequest) -> dict:
    ...
    results = await _validate(req.rows, default_source="csv")  # never trust the caller
    ...
    try:
        res = await insert_invoices([r.data for r in results], created_by=req.created_by)
...
async def upload_manual(inv: ManualInvoice) -> dict:
    ...
    result = (await _validate([row], default_source="manual"))[0]
    if not result.valid:
        raise HTTPException(422, {"message": "invoice is invalid", "errors": result.errors})
    try:
        res = await insert_invoices([result.data], created_by=created_by)
    except HasuraError as exc:
        raise HTTPException(502, f"Hasura rejected the insert: {exc}")
